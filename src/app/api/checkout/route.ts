...
export async function POST(req: NextRequest) {
  try {
    const { plan, name, email, phone, promoCode } = await req.json();
    ...
    if (existing?.active && existing.planStatus === "active") {
      if (existing.plan === "monthly" && planKey !== "monthly") {
        // Return conflict info so the UI can prompt the user to switch
    ...
    if (promoCode) {
      ...
      // If code not found, fall through — Stripe will catch it or user can retype
    }
    ...
    const sessionParams: Parameters<typeof stripe.checkout.sessions.create>[0] = {
      ...
      metadata: { plan: planKey, name, phone: phone ?? "", promoCode: promoCode ?? "" },
      subscription_data: {
